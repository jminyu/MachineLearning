...
%% Modification Data processing for train data

fprintf('modification data processing');
...
fprintf(1, 'Size of the training dataset=Approximatly %5d \n', totnum);
...
for k=1:numbatches
  batchdata(:,:,k) = digitdata(randomorder(1+(k-1)*batchsize:k*batchsize), :);
  batchtargets(:,:,k) = targets(randomorder(1+(k-1)*batchsize:k*batchsize), :);
end;

clear digitdata targets;

%% Modification Data processing for test data
digitdata=[];
targets=[];
